...
public static void ReporteVenta() {
    ...
    try {
        ...
        float[] columnsWidths = {5, 10, 10, 10, 20, 20, 15, 10};
...
public static void ReporteCotizaciones() {
    Document document = new Document();
    document.setPageSize(new Rectangle(830, document.getPageSize().getHeight()));
    ...
    try {
        ...
        float[] columnsWidths = {14, 40, 14, 25}; // Removed ID and Estado
        ...
        try {
            Connection cn = Conexion.obtenerConexion();
            String sql = "SELECT p.id, p.descripcion, dc.cantidad, p.precio_venta " +
                    "FROM producto p " +
            ...
            while (rs.next()) {
                tabla.addCell(rs.getString(1)); // ID del producto
                tabla.addCell(rs.getString(2)); // Descripción del producto
                tabla.addCell(rs.getString(3)); // Cantidad de detalle_compra
                tabla.addCell(rs.getString(4)); // Precio Venta del producto
            }
